...
latestDate=$(curl $latestUrl | awk -F '(setResponse\\(|\\);)' '// {print $2}' | jq '.table.rows[0].c[0].v')
latestDateCheck=$(sed 's/[.-]//g' <<< $latestDate)
latestTimeStamp=$(if [[ $latestDateCheck =~ ^[0-9]$]] && echo "$latestDateCheck" || echo "0")
...
awk -F '(LogNet:|name=| userId: NULL:| platform:)' '/userId/ {
    ...
    if ((dateCheck > '$latestTimeStamp')) {
        print("curl -s -o /dev/null --data-urlencode \"" arg1"\" --data-urlencode \"" arg2"\" --data-urlencode \"" arg3"\" --data-urlencode \"" arg4"\" '"$gPostURL);
